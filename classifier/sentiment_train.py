# -*- coding: utf-8 -*-
"""
Created on Mon Jul 23 17:19:41 2018

Taken from here: 
    https://streamhacker.com/tag/feature-extraction/
...
if __name__ == "__main__":
    ...
    # twitter dataset
    PATH = "model.pickle"
    DATA_PATH = "data\\tweets.csv"
    STOP_WORDS_PATH = "data\\stopwords.csv"
    
    dataset = pd.read_csv(DATA_PATH)
    stop_words = pd.read_csv(STOP_WORDS_PATH)
    
    X = list(dataset['tweet_text'])
    y = ['pos' if s == 4 else 'neg' for s in dataset['sentiment_label']]
    ...
    OUT_PATH = "model_toxic_comments.pickle"
    TRAIN_PATH = "data\\kaggle-toxic-comments\\train.csv"
    TEST_PATH = "data\\kaggle-toxic-comments\\test.csv"
    TEST_LABELS_PATH = "data\\kaggle-toxic-comments\\test_labels.csv"
    
    train_dataset = pd.read_csv(TRAIN_PATH)
    test_dataset = pd.read_csv(TEST_PATH)
    test_labels = pd.read_csv(TEST_LABELS_PATH)
    
    X_train = list(train_dataset['comment_text'])
    y_train = ['pos' if s==1 else 'neg' for s in train_dataset['toxic']]
    X_test = list(test_dataset['comment_text'])
    y_test = ['pos' if s==1 else 'neg' for s in test_labels['toxic']]
    
    model = build_and_evaluate(X_train, y_train, outpath=OUT_PATH)